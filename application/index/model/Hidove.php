...
//            'page'=>$pageInfo['page'],
    ...
    public static function search($wd){
        //获取源配置
        $Hidove = Hidove();
        //判断是否使用缓存
        if ($Hidove['cache-search']=='true'){
            # 获取缓存
            $data = Cache::tag('search')->get($wd);
            $dataType = '服务器缓存';
            if (!$data){
                $data = collect('array','list','','',$wd);
                $dataType = '中心服务器';
                # 判断获取数据是否成功，否则不进行缓存
                if ($data){
                    # 缓存到本地
                    Cache::tag('search')->set($wd,$data,$Hidove['cache-search-time']);
                }
            }
        }else{
            $data = collect('array','list','','',$wd);
